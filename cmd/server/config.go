...
type Config struct {
	...
	// APIKeys realm 专用密钥（可选）。用于"国内一套 key、国外一套 key"：
	// 持 api_keys.cn 的客户端只能看到/调用 cn: 模型，api_keys.global 同理。
	// api_key 仍是全通配（面板登录与旧客户端不受影响）。
	APIKeys struct {
		CN     string `json:"cn"`
...
func applyEnv(c *Config) {
	...
	if v := os.Getenv("WB2A_API_KEY_CN"); v != "" {
		c.APIKeys.CN = v
	}
	if v := os.Getenv("WB2A_API_KEY_GLOBAL"); v != "" {
		c.APIKeys.Global = v
	}
	if v := os.Getenv("WB2A_AUTH_DIR"); v != "" {
		c.AuthDir = v
